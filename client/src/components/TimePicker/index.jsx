...
function TimePicker({ hours, minutes, onTimeSelect }) {
   ...
   const onClick = (e) => {
      ...
      const scrollContainer =
         number.dataset.type === 'hours' ? hoursRef.current : minutesRef.current
   ...
   useEffect(() => {
      ...
      scrollToTime(
         hoursRef.current,
         hoursRef.current.querySelector(currentSelector)
      )
      scrollToTime(
         minutesRef.current,
         minutesRef.current.querySelector(currentSelector)
      )
   }, [])
   ...
   return (
      <div className="TimePicker Scroll" onClick={onClick}>
         <div ref={hoursRef} className="TimePicker__numbers">
            {numbers.map(
               (n) =>
                  n < 24 && (
                     <div
                        className={clsx('TimePicker__number', {
                           selected: selected.hours === n,
                        })}
                        key={n}
                        data-value={n}
                        data-type="hours"
         ...
         <div className="TimePicker__bottom">
            ...
            <div className="Link" onClick={onOk}>
               Оk
            </div>
         </div>
      </div>
