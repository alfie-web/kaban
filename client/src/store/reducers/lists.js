...
export const listsSlice = createSlice({
   ...
   reducers: {
      ...
      setCards: (state, { payload }) => {
         const findedList = state.items.find(
      ...
      setEditedCard: (state, { payload }) => {
         if (!payload) {
            ...
            if (list) {
               state.editedCard = list.cardItems.find(
                  (c) => c._id === payload.cardId
               )
            }
         }
      },
      ...
      setMoveCard: (state, { payload }) => {
         const { startListId, endListId, currentPosition, newPosition } =
            payload
         ...
         if (startListId === endListId) {
            // Если перемещаем карточку внутри текущего листа
            newCards.splice(
               newPosition,
               0,
               newCards.splice(currentPosition, 1)[0]
            )
         } else {
            // Если перемещаем карточку в другой лист
...
export const createCard = (listId, title) => async (dispatch, getState) => {
   ...
   try {
      const { data } = await cardsAPI.createCard({
         listId,
         title,
         position: finded.cardItems.length,
      })
